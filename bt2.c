...
{   
  ...
  //float wla = (mkt->stats.p_wins) ? (float) ((float)mkt->stats.p_wins / ((float)mkt->stats.p_wins + (float)mkt->stats.p_losses) * (float)100) : 0;
  ...
  snprintf(p, 299, "%s/%s-%s-%s/index.html", global.conf.logpath, mkt->exch->name,
      mkt->asset, mkt->currency);

  if((mkt->html = fopen(p, "w")) ==  NULL)
  {
    logger(C_BT, WARN, "bt_mkindex", "error creating html index '%s': %s (disabling plots)",
        p, strerror(errno));
    mkt->opt &= ~MKO_PLOT;
    return;
  }
...
bt_market(struct market *mkt)
{
  ...
  if(mkt->state & MKS_LONG)
  {
    mkt->profit =((mkt->buy_size - mkt->buy_fee) * lc->close)
      - ((mkt->buy_size - mkt->buy_fee) * mkt->buy_price);

    mkt->profit_pct = mkt->profit / ((mkt->buy_size - mkt->buy_fee) * mkt->buy_price) * 100;

    mkt->profit_pct_high =
        (mkt->profit_pct > mkt->profit_pct_high) ? mkt->profit_pct : mkt->profit_pct_high;

    mkt->profit_pct_low =
        (mkt->profit_pct < mkt->profit_pct_low) ? mkt->profit_pct : mkt->profit_pct_low;
  ...
  if(!(mkt->state & MKS_LONG) && mkt->a.advice == ADVICE_LONG)
  {
    ...
    o.price = mkt->buy_price = lc->close;
    mkt->buy_size = (mkt->stats.p_start_cur + mkt->stats.p_profit) / lc->close;
    o.fee = mkt->buy_fee = mkt->buy_size * (mkt->p_fee / 100);
    o.filled = mkt->buy_size - o.fee;

    mkt->state |= MKS_LONG;
    mkt->stats.p_fees += mkt->buy_fee;
    mkt->stats.p_trades++;
  ...
  {
    double buy_funds = mkt->buy_size * mkt->buy_price;
    double buy_total = mkt->buy_size - mkt->buy_fee;
    double sell_fee  = buy_total * (mkt->p_fee / 100);
    double sell_funds = (buy_total - sell_fee) * lc->close;
    double profit = sell_funds - buy_funds;
    double profit_pct = (sell_funds - buy_funds) / buy_funds * 100;

    o.filled = mkt->sell_size = buy_total - sell_fee;
    o.price = mkt->sell_price = lc->close;
    o.fee = mkt->stats.p_fees += sell_fee;

    mkt->profit = profit;
    mkt->sell_time = lc->time;
    mkt->state &= ~MKS_LONG;
    mkt->stats.p_trades++;
    mkt->stats.p_profit += profit;
...
bt_testcase(void *arg)
{
  ...
  for(x = 0; x < ds->hdr.init_candles - 2; x++)
  {
    ...
    // End test early if out of currency
    if(mkt->stats.p_start_cur + mkt->stats.p_profit < 0)
      break;
  }
        ...
        for(; z < bt_numthreads; z++)
        {
          ...
          if(state == BTT_DIE)  // test case completed
          {
                ...
                && (best_case == NULL || best_case->results.profit < t->bc->results.profit))
            {
              ...
              printf("prof: %f wins: %d losses: %d trades: %d wla: %.2f uargs:",
